...
```powershell
irm https://raw.githubusercontent.com/steekell/postinstall-windows/v0.1.4/setup-postinstall-windows.ps1 | iex
```

Le bootstrap télécharge l’archive du tag `v0.1.4` dans un répertoire temporaire, exécute `setup-postinstall.ps1` avec son catalogue et ses modules, puis supprime les fichiers temporaires. Pour une version différente, le tag et la valeur `$version` du bootstrap doivent correspondre.
...
```powershell
Invoke-WebRequest https://raw.githubusercontent.com/steekell/postinstall-windows/v0.1.4/setup-postinstall-windows.ps1 -OutFile .\setup-postinstall-windows.ps1
...
Pour rendre `v0.1.4` disponible, le dépôt GitHub `steekell/postinstall-windows` doit être public et le tag doit être poussé :
...
GitHub générera alors automatiquement l’archive `https://github.com/steekell/postinstall-windows/archive/refs/tags/v0.1.4.zip` utilisée par le bootstrap.
